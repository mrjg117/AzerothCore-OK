...
# 修正：在 cmake --build 后追加 ` -- -k 0`，把 -k 透传给底层 ninja（官方 Dockerfile 用 Ninja 生成器），
#       -k 0 = keep going until 0 jobs fail（无限继续），使其遇错继续编译，暴露所有模组错误。
#       注意：ninja 的 -k 必须带数字（make 可单独 -k，ninja 不行），空 -k 会报 "option requires an argument"。
...
if grep -qF 'cmake --build . --config "$CTYPE" -j' "$F"; then
  if grep -qF -- '-- -k' "$F"; then
    ...
    sed -i 's#cmake --build \. --config "$CTYPE" -j .* \\#cmake --build . --config "$CTYPE" -j $(($(nproc) + 1)) -- -k 0 \\#' "$F"
    echo "core keep-going: patched -> ninja -k 0 (expose all compile errors)"
